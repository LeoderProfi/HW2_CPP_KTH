...
template <typename PointT, typename RectangleT> class MplWriter {
public:
  MplWriter(std::ostream &os, double markerSize = 0.5) : m_os(os) {
    writePreamble(markerSize);
  }

  MplWriter(const std::string &filename, double markerSize = 0.5)
      : m_ofs(std::make_unique<std::ofstream>(filename)), m_os(*m_ofs) {
    writePreamble(markerSize);
  }
  ...
  MplWriter &operator<<(const RectangleT &rect) {
    m_os << "r = plt.Rectangle([" << rect.bottomLeft.x << ", "
         << rect.bottomLeft.y << "], " << (rect.topRight.x - rect.bottomLeft.x)
         << ", " << (rect.topRight.y - rect.bottomLeft.y) << ", fill=False)\n";
    m_os << "ax.add_patch(r)\n";
