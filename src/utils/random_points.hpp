...
/*
  ...
  sf::RandomPointGenerator<Point> generator; // Use default seed (not
  reproducible) sf::RandomPointGenerator<Point> generator{ 2565 }; // Use 2565
  as the seed (reproducible)

  generator.addNormalPoints(1000);                      // Adds 1000 standard
  normal random points generator.addNormalPoints(1000, PointT{2.0, 3.0});    //
  Adds 1000 normal random points with mean (2.0, 3.0) and unit variance
  generator.addUniformPoints(1000, PointT{ 4.0, 0.0 }); // Adds 1000 uniformly
  distributed points in the square 3 < x < 5, -1 < y < 1

  auto points = generator.takePoints(); // Gets the randomly generated points
  and resets the generator's points.
                                        // auto type is std::vector<PointT>

*/

template <class PointT> class RandomPointGenerator {
public:
  RandomPointGenerator() : m_rndGen(std::random_device{}()) {}

  RandomPointGenerator(size_t seed) : m_rndGen({seed}) {}
  ...
  void addNormalPoints(size_t n, const PointT &mean = PointT{0., 0.}) {
    addRandomPoints(n, std::normal_distribution<>(mean.x, 1.0),
                    std::normal_distribution<>(mean.y, 1.0));
  }

  void addUniformPoints(size_t n, const PointT &mean = PointT{0., 0.}) {
    addRandomPoints(
        n, std::uniform_real_distribution<>(mean.x - 1.0, mean.x + 1.0),
