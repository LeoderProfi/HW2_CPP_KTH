...
/*
  ...
  auto points = sf::readCsvPoints("swelakes.csv"); // auto type is
  std::vector<PointT>

*/

template <typename PointT>
inline std::vector<PointT> readCsvPoints(const std::string &filename) {
  ...
  while (std::getline(file, line)) {
    ...
    if (delimPos == std::string::npos) {
      throw std::runtime_error("File format error on line " +
                               std::to_string(numRecords + 1) +
                               "! (each line should contain two floating-point "
